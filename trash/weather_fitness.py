...
MODEL_PATH = "./models/wf_model.h5"
MEAN_PATH = "./models/wf_mean.pickle"
STD_PATH = "./models/wf_std.pickle"
CACHE_PATH = "./cache/weather_fitness"
...
def generate_dataset(predict_type,db_con,config):
    ...
    x,y = dataset2.load_dataset(db_con,features,["is_win","is_place","linfo_field_code"])
    x = x.drop(["linfo_field_code","linfo_field_status"],axis = 1)
    x_col = x.columns 
    y_col = y.columns
    con = concat(x,y)
    con = con[con["linfo_field_code"] != 0]
    x = con.loc[:,x_col]
    y = con.loc[:,y_col]
    del con

    y["linfo_field_code"] = y["linfo_field_code"] - 1
    y[target] = y["linfo_field_code"]
...
def create_model(activation = "relu",dropout = 0.8,hidden_1 = 10):
    nn = Sequential()
    nn.add(Dense(units=hidden_1,input_dim = 223))
