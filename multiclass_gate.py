...
def create_model(activation = "relu",dropout = 0.3,hidden_1 = 80,hidden_2 = 80,hidden_3 = 80):
    ...
    for i in range(5):
        #res = x
        #gate = ZeroPadding2D(padding = ((0,1),(0,0)))(x)
        gate = Conv2D(depth,(1,1),padding = "valid",kernel_initializer="he_normal",kernel_regularizer = l2(l2_coef))(x)
        #gate = BatchNormalization(axis = bn_axis,momentum = momentum)(gate)
        gate = Activation("sigmoid")(x)
